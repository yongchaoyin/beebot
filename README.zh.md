...
BeeBot 是一个跑在 macOS 上的本地电脑 Agent。你给出目标和边界，Bot 像蜂群
...
你。只跑一步 GUI 或浏览器的临时子任务工人，不是蜂群成员。
...
| 你说 | 产品应理解成 |
| --- | --- |
| 「大家把这件事完成。」 | 把目标和边界交给蜂群。他们自主协作，直到交付或需要你判断。 |
...
蜂群共用一台持久的 Linux 机器。这是共事的物理形态，不是比喻。
...
选厂商。上面的蜂群协作是北极星；今天的 Group 仍是群聊。目前仍是实验性项目，
