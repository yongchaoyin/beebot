...
界面采用中性灰白、深灰和少量蓝色，不再使用昆虫主题、黄色主调或相关隐喻。六种原创头像共享同一个真实状态驱动的动作引擎，源码界面与正式打包渲染器保持一致。头像编辑器中的「头像动效」支持「自然」「轻微」「关闭」；系统减少动态效果、窗口失焦和离屏规则优先。历史消息和群头像拼图保持静止，用户自定义照片和已保存配置继续兼容。详见[实现与验收说明](docs/implementation/presence-theme.md)。
