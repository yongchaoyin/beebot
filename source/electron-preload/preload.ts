...
export function createDesktopPreloadBridge(options: {
  ...
  const desktop: Record<string, any> = {
    ...
    agent: {
      ...
      getInferenceVendors: () => edge("getInferenceVendors"),
      upsertInferenceVendor: (request: Record<string, unknown>) => edge("upsertInferenceVendor", request),
      deleteInferenceVendor: (id: string) => edge("deleteInferenceVendor", { id }),
      getUiLanguage: () => edge("getUiLanguage"),
      setUiLanguage: (language: string) => edge("setUiLanguage", { language }),
      getInferenceRouter: () => edge("getInferenceRouter"),
      setInferenceRouter: (request: string | Record<string, unknown>) => edge("setInferenceRouter", typeof request === "string" ? { provider: request } : request),
