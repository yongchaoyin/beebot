...
export async function buildSandSendMessage<Context>(ctx: Context, input: SendMessageInput, deps: SendMessageDependencies<Context>): Promise<SandOutgoingMessage> {
  const raw = sendMessageParameters.parse(input); const reply = raw.reply_to || undefined, channel = raw.channel || undefined; const work = { ...(raw.work_on ? { work_on: raw.work_on } : {}), ...(raw.purpose ? {purpose: raw.purpose} : {}), ...(raw.collaboration ? {collaboration: raw.collaboration} : {}) };
  ...
  if (raw.type === "secret-request") { const secret = raw.secret; if (secret == null) throw new SandToolInputError("secret is required when type is secret-request"); return { type: "secret-request", secretRequest: { label: clampSecretLabel(secret.label), ...(secret.description ? { description: clampSecretDescription(secret.description) } : {}), target: { kind: "channel-credential", platform: secret.connector, field: secret.field } }, ...work, ...(reply ? { reply_to: reply } : {}) }; }
  const url = raw.url ?? "";
  try { if (new URL(url).protocol === "https:" && deps.classifyAttachment?.(url) === "file") return { type: "text", content: url, ...work, ...(reply ? { reply_to: reply } : {}), ...(channel ? { channel } : {}) }; } catch {}
  const source = await resolveAttachmentSource(ctx, url, deps);
  return { type: "attachment", url: source.url, ...work, ...(reply ? { reply_to: reply } : {}), ...(source.fileName ? { file_name: source.fileName } : {}), ...(raw.alt ? { alt: raw.alt } : {}), ...(channel ? { channel } : {}), ...await dimensions(source.url, deps) };
}
