...
export async function buildSandSendMessage<Context>(ctx: Context, input: SendMessageInput, deps: SendMessageDependencies<Context>): Promise<SandOutgoingMessage> {
  const raw = sendMessageParameters.parse(input); const reply = raw.reply_to || undefined, channel = raw.channel || undefined; const work = { ...(raw.work_on ? { work_on: raw.work_on } : {}), ...(raw.collaboration ? { collaboration: raw.collaboration } : {}), ...(raw.notify ? { notify: raw.notify } : {}) };
