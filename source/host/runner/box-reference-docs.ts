...
export const SAND_BOX_DEBUGGING_REFERENCE_DOC = [
  ...
  `- Next steps: retry first, since most failures are just a box still booting. You can't rebuild the box yourself, so if it's wedged or stuck on a stale image, surface a clear status and tell the user to recover it from Settings → Updates tab → "Update BeeBot's Computer" (its button says "Update") — it moves the box to a fresh instance while keeping files and logins, and can unstick a wedged box without data loss. That is the recovery action to point users at; the "Reset BeeBot's Computer" row below it restores from the last saved snapshot and can lose recent unsynced work, so never direct the user to it. request_box_help is for handing the user a manual step on a working desktop (a login or captcha), not a repair tool.`,
  "",
].join("\n");

export const SAND_APP_UI_REFERENCE_DOC = [
  "# The BeeBot app UI (real paths — never invent others)",
  "",
  `A compact map of BeeBot's real interface so you can guide the user or self-recover. Use only what's listed here; for anything else, follow "Never fabricate data" and say you're unsure rather than inventing a path.`,
  ...
  `- Deleting an agent: the user does this from the sidebar — right-click the agent's row and choose "Delete" (a permanent delete that removes the agent and its transcript, with a confirm). Hiding a Bot and permanently deleting it are different operations; use only the actions actually shown, and explain the effect before confirming deletion.`,
  "- Global Settings contains General, Servers, Router, Usage & Billing (when available), and Updates. Do not invent a separate Appearance or Team Setup tab.",
  "- General: model/server connection shortcuts, Theme (Follow System / Light / Dark), Language, and available execution safety preferences. BeeBot does not require a Cursor account for local model APIs or independently authorized Nodes.",
  "- Router → Model APIs: configure the model accounts for this computer. A Bot stores its selected API ID; adding another API does not repair a deleted binding. Open the Bot's settings and explicitly choose an existing API. Never silently substitute a provider or disclose keys.",
  "- Servers: connect independently deployed Nodes and manage the current device's authorization. A remote Bot uses its own Node's model configuration, not the desktop's Router. Connected does not mean model-ready or permission to use every Bot.",
  "- Plugins are reached from the sidebar's Plugins entry. Available catalogs, accounts and authentication are supplied by the configured integration. Do not assume that the desktop, all Nodes and external providers share accounts or credentials.",
  "- Updates and computer recovery are different operations. Use only controls exposed for the active runtime, inspect the actual confirmation and preserve user data. Do not promise official updates or a data-preserving reset when those are not available. Never recommend deleting configuration, keys or Docker volumes to fix a missing model binding.",
