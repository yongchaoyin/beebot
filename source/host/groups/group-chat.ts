import { COLLABORATION_GUIDANCE, COLLABORATION_DELIVERY_GUIDANCE, type ChatIntent } from "../runner/tools/collaboration-schema.js";
...
export interface GroupMember { id: string; name: string; description: string } export interface GroupDescription { name: string; description: string } export type GroupMessage = { intent?: ChatIntent; collaborationTargets?: readonly string[]; id?: string; replyToId?: string; replyToMemberId?: string; workOnId?: string; awaitingUser?: boolean; speaker: { kind: "user"; name?: string } | { kind: "member"; id: string; name: string }; content: string };
...
export function resolveMessageResponders<T extends Pick<GroupMember, "id" | "name">>(members: readonly T[], messages: readonly GroupMessage[]): T[] {
  ...
  for (const message of messages) {
    if (message.awaitingUser || isPassContent(message.content) || (message.speaker.kind === "member" && message.intent === "update")) continue;
    if (message.speaker.kind === "member" && message.collaborationTargets) {
      for (const id of message.collaborationTargets) if (members.some(member => member.id === id)) selected.add(id);
      continue;
    }
