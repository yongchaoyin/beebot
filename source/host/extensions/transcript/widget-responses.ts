import { isWorkQuestionCurrent } from "./work-question-context.js";
...
export class WidgetResponses {
  ...
  async respondToWidget(
    ...
    const decisionContext = currentEntry.decisionContext as any;
    const staleContext = decisionContext?.kind === "work"
      ? !isWorkQuestionCurrent(targetSession, decisionContext)
      : decisionContext && decisionContext.userMessageId !== (latestUser?.id ?? null);
    if (staleContext) {
