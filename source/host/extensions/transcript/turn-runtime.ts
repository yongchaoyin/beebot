    ...
    switch (update.type) {
      ...
      case "send-message": {
        ...
        if (threaded.collaboration) {
          ...
          const work = prepareCollaboration({actorRole:this.tm.botRoles?.read(runSession.id) ?? null,
            uncertainMessageIds:this.tm.sendPipeline.deliveries.list(runSession.dbPath).filter((record: {state: string}) => record.state === "needs-review").map((record: {id: string}) => record.id),
            messageId: sendId, dbPath: runSession.dbPath, actor: runSession.id, members: [runSession.id], entries: runSession.db.getTranscriptEntries(), message: threaded});
