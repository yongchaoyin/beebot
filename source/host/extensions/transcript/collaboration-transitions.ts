...
export function workIsAccepted(task: CollaborationTask, tasks: ReadonlyMap<string, CollaborationTask>, seen = new Set<string>()): boolean {
  ...
  return task.dependencies.every(id => {
    ...
    return !!dep && task.submission!.dependencyVersions.some(pin => pin.id === id && pin.version === dep.version) && workIsAccepted(dep, tasks, next);
  });
}
export function workDependenciesReady(task: CollaborationTask, tasks: ReadonlyMap<string, CollaborationTask>): boolean {
  return task.dependencies.every(id => {const dep = tasks.get(id);return !!dep && workIsAccepted(dep, tasks);});
}
...
export function advanceWork(args: {
  ...
  if (action.action === "revise") {
    ...
    next.revisionSourceId = source.id;next.state = "offered";delete next.claimedBy;delete next.reason;delete next.submission;delete next.review;
    ...
    if (action.action === "claim") {
      check(["offered", "waiting"].includes(prior.state) || (prior.state === "accepted" && !workIsAccepted(prior, tasks)), "work_not_claimable", "This work is already claimed. Do not start another concurrent run.");
      check(workDependenciesReady(prior, tasks), "work_dependencies_pending", "Prerequisite work must pass review before this claim.");
      next.state = "claimed";next.claimedBy = actor;delete next.reason;delete next.submission;delete next.review;
    } else if (action.action === "wait") {
      check(["offered", "claimed", "blocked", "changes-requested"].includes(prior.state) || (prior.state === "accepted" && !workIsAccepted(prior, tasks)), "work_not_waitable", "Work already waiting or submitted must not be replayed.");
      ...
      if (action.action === "submit") {
        ...
        next.state = "review";delete next.review;delete next.reason;wake = [prior.reviewer];
