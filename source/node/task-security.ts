...
export class TaskSecurityLedger {
  constructor(private readonly db: DatabaseSync, private readonly records: RecordCodec = plaintextRecords) {
    db.exec(`
  ...
  private freezeId(id: string, principal: string, scope: string, target: string, active: number): string {
    return JSON.stringify([id, principal, scope, target, active]);
  }
  private encodeFreeze(value: TaskFreeze): string {
    return this.records.encode("task_freezes", this.freezeId(value.id, value.principalId, value.scope, value.target, value.releasedAt === null ? 1 : 0), value);
  }
  private decodeFreeze(row: Record<string, unknown>): TaskFreeze {
    return this.records.decode<TaskFreeze>("task_freezes", this.freezeId(String(row.id), String(row.principal), String(row.scope), String(row.target), Number(row.active)), String(row.data));
  }
  record(goalId: string, principalId: string, botId: string, acceptedAt: number, source?: TaskSource): void {
    ...
    this.db.prepare("INSERT INTO task_authorizations(goal_id,data) VALUES (?,?)").run(goalId, this.records.encode("task_authorizations", goalId, grant));
  }
  authorization(goalId: string): TaskAuthorization | undefined {
    ...
    return row ? this.records.decode<TaskAuthorization>("task_authorizations", goalId, String(row.data)) : undefined;
  }
  active(principal: string, scope: TaskFreeze["scope"], target: string): TaskFreeze | undefined {
    const row = this.db.prepare("SELECT * FROM task_freezes WHERE principal=? AND scope=? AND target=? AND active=1").get(principal, scope, target);
    return row ? this.decodeFreeze(row) : undefined;
  }
  list(principal: string): TaskFreeze[] {
    return this.db.prepare("SELECT * FROM task_freezes WHERE principal=? ORDER BY rowid DESC LIMIT 200").all(principal).map(row => this.decodeFreeze(row));
  }
  get(id: string, principal: string): TaskFreeze | undefined {
    const row = this.db.prepare("SELECT * FROM task_freezes WHERE id=? AND principal=?").get(id, principal);
    return row ? this.decodeFreeze(row) : undefined;
  }
  freeze(principal: string, actorSessionId: string, scope: TaskFreeze["scope"], target: string): TaskFreeze {
    ...
    this.db.prepare("INSERT INTO task_freezes VALUES (?,?,?,?,?,1)").run(record.id, principal, scope, target, this.encodeFreeze(record));
    return record;
  }
  revoke(goalId: string, freeze: TaskFreeze): void {
    ...
    this.db.prepare("UPDATE task_authorizations SET data=? WHERE goal_id=?").run(this.records.encode("task_authorizations", goalId, { ...grant, revokedAt: freeze.requestedAt, freezeId: freeze.id }), goalId);
  }
  release(freeze: TaskFreeze): TaskFreeze {
    const next = { ...freeze, releasedAt: Date.now(), version: freeze.version + 1 };
    this.db.prepare("UPDATE task_freezes SET active=0,data=? WHERE id=?").run(this.encodeFreeze(next), freeze.id);
    return next;
  }
}
