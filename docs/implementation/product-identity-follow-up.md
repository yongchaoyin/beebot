...
原始归档的图片不作为 BeeBot 页面/安装背景使用；九个已替代图片从输出删除的
清单和字节校验仍然生效。现有许可证、来源、重建证据不删，不篡改用户消息及历史。
...
node --test tests/product-identity.test.mjs tests/product-identity-boundaries.test.mjs \
  tests/product-branding.test.mjs tests/router-settings.test.mjs
npm run check
...
六个供应商及旧 provider 用量 UI 和关闭按钮定位。

本地相关单元/组件回归 **105 项通过，0 失败、0 取消、0 跳过**，包含新增 16 项；前后端 TypeScript 与前端构建通过。这些是完整套件的子集，不与 CI 计数相加。

本地 Playwright/Chromium 本轮 **351 项断言通过**，0 页面错误、0 console error/warning。
包含 1100×760 与 390×650、源码 GeneralSettingsPanel、正式版实际替换的 Vs、中英/浅深色、
真实 Enter 导航、关闭按钮操作位、旧外部会话确认与退出、用户草稿原文不变；另检验实际
