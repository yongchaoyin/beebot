...
export function SignedInOnboardingStep({ step, draft, dailyTools, meetBeat = 0, demoBeat = 0, jobsBeat = 0, error = null, computer = EMPTY_COMPUTER, isAvatarLanded = false, renderCharacter, onAvatarSlotAttached, onDraftChange, onDailyToolsChange, onBack, onForward, onCreate }: {
  ...
  if (step === "meet") return <StepLayout step={step} title="Meet BeeBot" footer={<Footer onForward={onForward} />}><div aria-hidden="true" className="sand-onboarding__composer"><p>{typedMeetText(meetBeat)}{meetBeat >= ONBOARDING_MEET_DELAY_BEATS && meetBeat < ONBOARDING_MEET_DELAY_BEATS + MEET_TYPED_TEXT.length ? <span style={{ borderLeft: "1px solid currentColor", display: "inline-block", height: "1em" }} /> : null}</p><span><button aria-label="Attach" tabIndex={-1} type="button">+</button><button aria-label="Send" onClick={onForward} tabIndex={-1} type="button">↑</button></span></div></StepLayout>;
  if (step === "computer-demo") return <StepLayout step={step} title="BeeBot has its own computer and works just like you" footer={<Footer onBack={onBack} onForward={onForward} />}><ComputerDemo beat={demoBeat} /></StepLayout>;
