import type { AvatarActivity } from "./avatar-state";
...
export function workLabel(agent: WorkState): WorkLabel | null {
  if (agent.isTransportDown === true) return label("unknown", "连接已断开", "Disconnected");
  ...
  if (agent.hasError === true) return label("error", "执行异常", "Action failed");
  ...
  if (agent.isPaused === true) return label("waiting", "已暂停", "Paused");
