...
export function createPresenceMotionSetting(React: typeof ReactAPI) {
  ...
  return function MotionSetting() {
    ...
    return h("div", { className: "bb-motion-setting" },
      h("div", null, h("label", { htmlFor: id }, zh ? "头像动态" : "Avatar motion"), h("small", { id: `${id}-help` }, zh ? "始终遵循系统减少动态效果设置" : "Always respects Reduce Motion")),
      h("select", { id, "aria-describedby": `${id}-help`, value: preference, onChange: (event: ReactAPI.ChangeEvent<HTMLSelectElement>) => setAvatarMotionPreference(event.currentTarget.ownerDocument, event.currentTarget.value as AvatarMotionPreference) },
