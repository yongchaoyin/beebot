...
const dataDir = await realpath(await mkdtemp(path.join(os.tmpdir(), "beebot-packaged-smoke-")));
...
try {
  ...
  const initialized = await postForm("/setup", { flow_id: setup.flow_id, csrf: setup.csrf, username: "owner", password }, setup.cookie);
  assert.equal(initialized.status, 200);
  const recovery = /data-recovery-code>([^<]+)</.exec(await initialized.text())?.[1];
  assert.ok(recovery, "Fresh smoke Node must supply first-device recovery material");
  ...
  const consent = await postForm("/oauth/authorize", { flow_id: authorization.flow_id, csrf: authorization.csrf, username: "owner", password, decision: "allow", recovery_code: recovery, confirm_recovery: "yes" }, authorization.cookie);
  ...
  await stop();
  const recoveryFile = path.join(dataDir, "offline-recovery.json");
  const recoveryOutput = await promisify(execFile)(process.execPath, [entry, "recovery-codes", "--data-dir", dataDir, "--output", recoveryFile, "--confirm-recovery"]);
  const recoveryData = JSON.parse(await readFile(recoveryFile, "utf8"));
  assert.equal(recoveryData.codes.length, 8); assert.equal((await stat(recoveryFile)).mode & 0o777, 0o600);
  for (const code of recoveryData.codes) assert.ok(!(recoveryOutput.stdout + recoveryOutput.stderr).includes(code), "CLI never logs recovery material");
  await start();
