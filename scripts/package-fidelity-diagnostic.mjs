...
const appName = `BeeBot Fidelity Diagnostic-${shortHash}.app`;
...
await run(SYSTEM_TOOLS.plutil, ["-replace", "CFBundleIdentifier", "-string", `com.yongchaoyin.beebot.fidelity.diagnostic.build${shortHash}`, infoPlist]);
await run(SYSTEM_TOOLS.plutil, ["-replace", "CFBundleDisplayName", "-string", `BeeBot Fidelity Diagnostic-${shortHash}`, infoPlist]);
