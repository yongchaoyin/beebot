...
export function patchPresenceRenderer(source) {
  ...
  result = once(result, 'function Y_t(n){const{from:e,to:t,angle:s}=K_t(n);return`linear-gradient(${s+90}deg, ${e}, ${t})`}', 'function Y_t(n){const c=RPresenceUI.avatarColors[n]??RPresenceUI.avatarColors.blue;return`linear-gradient(${c},${c})`}', "shared original color swatches");
  result = once(result, 'k=Ij.map(I=>{const P=i&&m===I,', 'k=RPresenceUI.avatarShapes.map(I=>{const P=i&&RPresenceUI.characterVariant(m)===RPresenceUI.characterVariant(I),', "eight original avatar choices");
  ...
  result = once(result, 'A=p.jsxs("div",{className:f,children:[v,E]})', 'A=p.jsxs("div",{className:f,children:[p.jsx(RPresenceAvatarPreview(),{color:u,shape:m}),v,E,p.jsx(RPresenceMotionSetting(),{})]})', "live editor and motion preference");
