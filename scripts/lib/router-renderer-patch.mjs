import { patchUiLanguageRenderer, uiLanguageBindings } from "./ui-language-renderer-patch.mjs";
...
const RRouterProviders=[
  {value:"claude-code",label:"Claude Code",description:"Use your existing Claude Code sign-in and BeeBot's connected plugins.",kind:"local",localKey:"claude-code"},
  {value:"codex",label:"Codex",description:"Use your existing ChatGPT sign-in from Codex with BeeBot's connected plugins.",kind:"local",localKey:"codex"},
...
function RRouterNumber(s){return new Intl.NumberFormat(globalThis.window?.__sandUiLanguage==="zh"?"zh-CN":"en-US").format(s)}
...
function RRouterUsageRows({usage:s}){return a.jsxs("div",{children:[a.jsx(ie,{label:"Requests",variant:"card",children:a.jsx(se,{as:"span",color:"secondary",size:"sm",children:RRouterNumber(s.requests)})}),a.jsx(ie,{divided:!0,label:"Input tokens",variant:"card",children:a.jsx(se,{as:"span",color:"secondary",size:"sm",children:RRouterNumber(s.inputTokens)})}),a.jsx(ie,{divided:!0,label:"Output tokens",variant:"card",children:a.jsx(se,{as:"span",color:"secondary",size:"sm",children:RRouterNumber(s.outputTokens)})}),a.jsx(ie,{divided:!0,label:"Cache tokens",variant:"card",children:a.jsx(se,{as:"span",color:"secondary",size:"sm",children:RRouterNumber(s.cacheReadTokens+s.cacheWriteTokens)})}),a.jsx(ie,{divided:!0,label:"Last used",variant:"card",children:a.jsx(se,{as:"span",color:"secondary",size:"sm",children:s.lastUsedAt?new Date(s.lastUsedAt).toLocaleString(globalThis.window?.__sandUiLanguage==="zh"?"zh-CN":"en-US"):"Not used yet"})})]})}
function RBoxRuntime(){const[s,e]=de.useState({mode:"remote",status:null,error:null,busy:!0});de.useEffect(()=>{let t=!0;window.desktop.agent.getBoxRuntime().then(n=>{t&&e({...n,error:null,busy:!1})}).catch(n=>{t&&e(r=>({...r,error:String(n?.message??n),busy:!1}))});return()=>{t=!1}},[]);const t=s.mode==="local-docker",n=async()=>{const r=t?"remote":"local-docker";e(i=>({...i,mode:r,busy:!0,error:null}));try{const i=await window.desktop.agent.setBoxRuntime(r);e({...i,error:null,busy:!1})}catch(i){e(o=>({...o,mode:t?"local-docker":"remote",error:String(i?.message??i),busy:!1}))}};return a.jsxs("div",{children:[a.jsx(ie,{description:t?((["Local Docker VM is ready.","Container is starting.","Local Docker VM is stopped.","Ready to create the local VM.","Docker is not running.","Docker is not installed.","Container grok-bot-local-vm exists but is not owned by BeeBot."].includes(s.status?.detail)&&typeof window!=="undefined"&&typeof window.__beebotUiLanguage?.text==="function"?window.__beebotUiLanguage.text(s.status.detail):s.status?.detail)??"Shell, files and computer use run in a Docker container on this Mac."):"This uses the external provider's hosted computer, not a separately connected BeeBot server. Provider access is required.",label:"Use local Docker VM",variant:"card",children:a.jsx("button",{"aria-checked":t,"aria-label":"Use local Docker VM",disabled:s.busy,onClick:n,role:"switch",style:{appearance:"none",background:t?"var(--color-accent-primary, #4f8cff)":"rgba(255,255,255,.14)",border:0,borderRadius:999,cursor:s.busy?"wait":"pointer",height:22,opacity:s.busy?0.65:1,padding:2,position:"relative",transition:"background .15s ease",width:38},type:"button",children:a.jsx("span",{style:{background:"white",borderRadius:"50%",boxShadow:"0 1px 3px rgba(0,0,0,.35)",display:"block",height:18,transform:"translateX("+(t?16:0)+"px)",transition:"transform .15s ease",width:18}})})}),s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:s.error}):null]})}
function RLanguageRow(){
  const api=window.__beebotUiLanguage,language=de.useSyncExternalStore(api.subscribe,api.snapshot,api.snapshot),[pending,setPending]=de.useState(false),[error,setError]=de.useState(false);
  const label=language==="zh"?"语言":"Language";
  const change=async next=>{if(pending||next===language)return;setPending(true);setError(false);try{await api.set(next)}catch{setError(true)}finally{setPending(false)}};
  return a.jsxs("div",{children:[a.jsx(ie,{divided:!0,label,variant:"card",children:a.jsx(ye,{"aria-label":label,disabled:pending,onValueChange:next=>{if(next==="en"||next==="zh")void change(next)},options:[{value:"en",label:"English"},{value:"zh",label:"中文"}],placement:"bottom-end",size:"lg",value:language,variant:"filled"})}),error?a.jsx("p",{role:"alert",children:language==="zh"?"语言未能保存，请重试。":"Language could not be saved. Please try again."}):null]})
}
function RServersPanel({onOpenBot}){
  ...
  de.useEffect(()=>{
    ...
    if(typeof mount!=="function"){host.current.textContent=window.__sandUiLanguage==="zh"?"此版本暂不支持服务器连接。":"Server connections are not available in this build.";return}
...
function RRouterPanel(){const[s,e]=RRouterState(),[t,n]=RRouterSecrets(),r=RRouterProviders.find(i=>i.value===s.provider)??RRouterProviders[0],i=s.usage?.providers?.[s.provider]??RRouterEmptyUsage,o=r.value==="codex"?"Uses the private ChatGPT login already stored by Codex on this Mac. Requests are made by BeeBot directly.":r.kind==="local"?"Uses Claude Code's existing login on this Mac.":"Stored on this Mac and used for OpenAI-compatible requests.";return a.jsx(Te,{children:a.jsxs("div",{className:k("sand-settings-general","sand-9f619 sand-78zum5 sand-dt5ytf sand-3qzy4x"),children:[a.jsx(re,{title:"Routing",children:a.jsx(ie,{description:r.description,label:"Provider",variant:"card",children:a.jsx(ye,{"aria-label":"Routing provider",onValueChange:l=>{if(l!==null)void e(l)},options:RRouterOptions,placement:"bottom-end",size:"lg",value:s.provider,variant:"filled"})})}),a.jsx(re,{title:"Computer",children:a.jsx(RBoxRuntime,{})}),a.jsx(re,{title:(window.__sandUiLanguage||"en")==="zh"?"模型 API":"Model APIs",children:a.jsx(RVendorAccounts,{})}),r.kind==="http"?null:a.jsx(re,{title:"Account",children:a.jsx(ie,{description:o,label:"Status",variant:"card",children:a.jsx(RRouterCredential,{provider:r,state:s,keys:t,onSaved:n})})}),s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:s.error}):null,a.jsx(re,{title:${'`Usage for ${r.label}`'},children:a.jsx(RRouterUsageRows,{usage:i})})]})})}
function RRouterUsageSummary({provider:s,usage:e,current:t,divided:n}){const r=${'`${RRouterNumber(e.requests)} requests · ${RRouterNumber(e.inputTokens)} input · ${RRouterNumber(e.outputTokens)} output · ${RRouterNumber(e.cacheReadTokens+e.cacheWriteTokens)} cached`'},i=t?"Current route":e.lastUsedAt?new Date(e.lastUsedAt).toLocaleString(globalThis.window?.__sandUiLanguage==="zh"?"zh-CN":"en-US"):"Not used yet";return a.jsx(ie,{divided:n,description:r,label:s.label,variant:"card",children:a.jsx(se,{as:"span",color:t?"primary":"secondary",size:"sm",children:i})})}
function RRouterUsage(){const[s]=RRouterState(),e=RRouterProviders.find(t=>t.value===s.provider),t=RRouterProviders.filter(n=>n.value===s.provider||(s.usage?.providers?.[n.value]?.requests??0)>0);return a.jsxs("div",{className:k("sand-usage-section","sand-9f619 sand-78zum5 sand-dt5ytf sand-ou54vl"),children:[a.jsx(re,{title:"Current provider",children:a.jsx(ie,{description:e?.description??"Choose a configured model provider in Router settings. This saved provider is not active.",label:e?.label??"Provider is not active",variant:"card",children:e?a.jsx(se,{as:"span",color:"secondary",size:"sm",children:"Selected"}):null})}),a.jsx(re,{title:"Tracked activity",children:a.jsx("div",{children:t.map((n,r)=>a.jsx(RRouterUsageSummary,{provider:n,usage:s.usage?.providers?.[n.value]??RRouterEmptyUsage,current:n.value===s.provider,divided:r>0},n.value))})})]})}
`;
...
export function patchOriginalSettingsPanel(source) {
  ...
  return patchProductConnections(patched);
}
...
export async function applyOriginalRendererRouterPatch({ stageRoot }) {
  ...
  const landingPath = landingCandidates[0].target;
  const accessBefore = await readFile(landingPath, "utf8");
  const accessAfter = patchProductAccessCopy(accessBefore);
  await writeFile(landingPath, accessAfter);
  changes.push({ role: "external-service-access-copy", path: `dist/renderer/assets/${landingCandidates[0].name}`,
    original: { bytes: Buffer.byteLength(accessBefore), sha256: sha256(accessBefore) },
    patched: { bytes: Buffer.byteLength(accessAfter), sha256: sha256(accessAfter) } });
  const branding = await brandRendererAssets(assetsRoot);
  const retiredAssets = await retireUnusedBrandAssets(assetsRoot, { rendererRoot: path.join(stageRoot, "dist", "renderer") });
  const localization=[];
  for(const name of (await readdir(assetsRoot)).sort()){
    if(!name.endsWith(".js"))continue;
    const target=path.join(assetsRoot,name),before=await readFile(target,"utf8");
    const bindings=uiLanguageBindings(before,name);
    // Limit this compiler ABI adapter to the verified upstream React chunks.
    if(!bindings.main&&!bindings.cacheBinding)continue;
    const result=patchUiLanguageRenderer(before,bindings);
    if(result.source===before)continue;
    await writeFile(target,result.source);
    changes.push({role:"ui-language",path:`dist/renderer/assets/${name}`,original:{bytes:Buffer.byteLength(before),sha256:sha256(before)},patched:{bytes:Buffer.byteLength(result.source),sha256:sha256(result.source)}});
    localization.push({path:`dist/renderer/assets/${name}`,copy:result.translated});
  }
  const record = {
    localization,
    branding,
