...
async function inventory(root, prefix = "") {
  ...
  for (const entry of await readdir(path.join(root, prefix), { withFileTypes: true })) {
    ...
    else throw new Error(`Unsupported Presence renderer entry: ${relative}`);
  }
  return result.sort();
}
...
export async function verifyPresenceRendererArchive({ archivePath, sourceRendererRoot, extension }) {
  const stage = await mkdtemp(path.join(tmpdir(), "beebot-presence-verify-"));
  try {
    ...
    if (JSON.stringify(extension) !== JSON.stringify(rebuilt)) throw new Error("Presence extension differs from the reproducible source build");
    ...
    if (JSON.stringify(expected) !== JSON.stringify(actual)) throw new Error("Presence packaged inventory contains missing or undeclared files");
    for (const relative of expected) {
      const wanted = await readFile(path.join(root, relative));
      const packaged = extractFile(archivePath, `dist/renderer/${relative}`);
      if (packaged.length !== wanted.length || hash(packaged) !== hash(wanted)) throw new Error(`Presence packaged bytes differ at ${relative}`);
    }
    return { fileCount: expected.length, presence: rebuilt.presence, chatAssets: rebuilt.chatAssets, chunks: rebuilt.chunks };
  } finally {
