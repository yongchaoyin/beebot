...
export function patchPresenceCreatePicker(source) {
  ...
  once("  let appearanceOpen=false;\n", "", "appearance is visible on creation");
  once('    appearanceOpen=root.querySelector("details")?.open??appearanceOpen;\n', "", "remove collapsed-state read");
  once("alive=false;refreshSerial++;unsubscribe?.();", "alive=false;refreshSerial++;root.__bbAvatarPicker?.destroy();root.__bbAvatarPicker=null;unsubscribe?.();", "dispose preview on close");
