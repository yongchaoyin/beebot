...
function rig(t){
 ...
 const calls=[];w.desktop={agent:{getInferenceVendors:()=>read()}};w.__sandUpdateAgent=async(...args)=>calls.push(args);
...
test("failed save retains old binding, does not change cached model or drop Bot description",async t=>{
 ...
 assert.equal(x.select().value,"removed-api");assert.equal(x.w.__sandAgentVendors["bot-a"],"removed-api");assert.match(x.hint(),/Save failed/);assert.equal(x.w.document.querySelector("textarea").value,"Keep the original description");
});
