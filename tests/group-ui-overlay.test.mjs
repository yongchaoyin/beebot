...
test("open group paints a member bar and @ mention menu", async () => {
  ...
  form.className = "sand-prompt-form";
  form.append(area); document.body.append(form);
  area.focus(); area.value = "@Wr"; area.setSelectionRange(3, 3);
  area.dispatchEvent(new window.Event("input", { bubbles: true }));
  const mention = document.getElementById("sand-beebot-mention");
  assert.ok(mention);
  assert.match(mention.textContent ?? "", /@Writer/);
  assert.equal(mention.getAttribute("role"), "listbox");
  assert.equal(area.getAttribute("aria-expanded"), "true");
  ...
  assert.equal(document.getElementById("sand-beebot-mention"), null, "remote selection closes stale group suggestions");
  assert.equal(area.getAttribute("aria-expanded"), null, "editor ARIA is restored");
