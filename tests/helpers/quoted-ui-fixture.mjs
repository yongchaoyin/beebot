...
export function quotedUIFixture() {
  return building ??= (async () => {
    ...
    const names=new Set(['pCn','uPn','ide','Kvn','RQuoteComponents']);
    ...
    const source=`
      import * as S from 'react';import * as p from 'react/jsx-runtime';import {createRoot} from 'react-dom/client';import {createPortal} from 'react-dom';
      const listeners=new Set();let version=0,entries=new Map(),scope='group',originalId='assignment',navImpl=null;
      ...
      function r1(){S.useSyncExternalStore(store.subscribe,store.get,store.get);return{agentId:scope,resolveEntry:id=>entries.get(id),revealQuotedEntry:navigation,openThread:id=>calls.push(['thread',id])};}
      ...
      function View(){S.useSyncExternalStore(store.subscribe,store.get,store.get);return p.jsxs(S.Fragment,{children:[
        mode==='link'?p.jsx(uPn,{entry:entries.get(originalId),label:'assignment'}):p.jsx(pCn,{targetId:originalId}),
      ...
      window.quoteFixture={calls,mount(host,record){entries=new Map([[record.id,record]]);originalId=record.id;quotePreview=ide(record);root=createRoot(host);root.render(p.jsx(View,{}));},
        ...
        preview:ide,components:()=>RQuoteComponents(),setPreview(value){quotePreview=value;publish()}}
    `;
