...
const validRole = {primaryJob:"实现聊天交互",responsibilities:[],outOfScope:[],deliverables:[],workingStyle:""};
...
export function mountSheet(host){const root=createRoot(host);let language='zh',vendors=[{id:'api-1',label:'Test API'}],drafts=[];
 ...
 return{drafts,update(next){language=next.language??language;vendors=next.vendors??vendors;render()},input(node,value){flushSync(()=>{Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set.call(node,value);node.dispatchEvent(new Event("input",{bubbles:true}));node.dispatchEvent(new Event("change",{bubbles:true}));})},click(node){flushSync(()=>node.click())},unmount(){flushSync(()=>root.unmount())}}}
` } });
function rig(t) {
...
test("actual React New Bot sheet submits selected identity and retains it on API and language refresh", t => {
  ...
  app.input(host.querySelector("#create-primary-job"), "实现聊天交互");
  app.click([...host.querySelectorAll("button")].find(button => button.textContent.trim() === "Get started"));
  assert.deepEqual(clean(app.drafts), [{ name: "New Bot", avatarColor: "violet", avatarShape: "cloud", inferenceVendorId: "api-1", role: validRole }]);assert.deepEqual(errors, []);
});
...
function nativeRig(t) {
  ...
  window.eval(roleSource);
  window.eval(patched.slice(0, patched.indexOf(groupMarker)));
  const pick = window.__sandPickCreateBot;
  window.__sandPickCreateBot = (preset,options) => pick({...preset,role:validRole},options);
  return context;
}
...
test("native New Bot exposes choices immediately and submits them without repainting the name field", async t => {
  ...
  assert.equal(host.querySelector(".bb-create-appearance"), null);assert.ok(host.querySelector("[data-role-field=primaryJob]"), "role form remains available");assert.equal(host.querySelectorAll('[role="radio"]').length, 19);
...
test("native patch is scoped, preserves Group and transport code, and fails on unknown anchors", () => {
  ...
  assert.throws(() => patchPresenceCreatePicker(native.replace("  let appearanceOpen=false,roleDetailsOpen", "  let appearanceOpen=true,roleDetailsOpen")), /anchor mismatch/);
