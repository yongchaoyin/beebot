...
test("creation picker exposes six distinct shapes and eleven neutral colors without a second action system", t => {
  ...
  assert.equal(host.querySelectorAll('[data-kind="shape"] [role="radio"]').length, 6);
  assert.equal(host.querySelectorAll('[data-kind="color"] [role="radio"]').length, 11);
  assert.equal(new Set([...host.querySelectorAll('[data-kind="shape"] [data-part="body"]')].map(node => node.getAttribute("d"))).size, 6);
...
test("shape/color choices are independent across all 66 combinations and preserve the SVG and draft nodes", t => {
  ...
  for (const shape of ["blob", "tablet", "pebble", "wedge", "cloud", "squircle"]) for (const color of ["black", "brown", "red", "orange", "yellow", "green", "cyan", "blue", "violet", "magenta", "gray"]) {
...
test("keyboard, localization, pending lock and disposal preserve selection without creating a Bot", t => {
  ...
  assert.deepEqual(changed.at(-1), { shape: "tablet", color: "blue" });
  assert.equal(window.document.activeElement, choose(host, "shape", "tablet"));
  ...
  assert.equal(host.querySelectorAll('[role="radio"]:disabled').length, 17);
  picker.setDisabled(false);const color = choose(host, "color", "red");color.click();assert.deepEqual(changed.at(-1), { shape: "tablet", color: "red" });
...
test("legacy shape identity survives color edits and remounting displays the same selection", t => {
  ...
  assert.equal(choose(host, "shape", "tablet").getAttribute("aria-checked"), "true");
...
test("native New Bot exposes choices immediately and submits them without repainting the name field", async t => {
  ...
  assert.equal(host.querySelector("details"), null);assert.equal(host.querySelectorAll('[role="radio"]').length, 17);
...
test("native pending failure locks appearance then preserves it for retry", async t => {
  ...
  host.querySelector('[data-create-field="submit"]').click();await tick();assert.equal(host.querySelectorAll('[role="radio"]:disabled').length, 17);
