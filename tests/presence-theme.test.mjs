...
import { installPresence } from "../frontend/src/presence/install.ts";
import { workLabel } from "../frontend/src/presence/status.ts";
import { characterLayers, characterVariant, createCharacterSvg } from "../frontend/src/presence/avatar-art.ts";
...
const css = await readFile(new URL("../frontend/src/presence/presence.css", import.meta.url), "utf8");
const palette = chunk => Object.fromEntries([...chunk.matchAll(/--bb-([\w-]+): (#[\da-f]{6});/gi)].map(match => [match[1], match[2]]));
const luminance = hex => {
...
for (const [index, mode] of ["light", "dark"].entries()) {
  test(`Presence ${mode} text and focus contrast cover actual surfaces`, () => {
...
test("theme installation is idempotent and leaves preference, draft, focus, scroll and listeners alone", async t => {
  ...
  installPresence(document); installPresence(document);
...
test("CSS keeps reduced motion, forced colors, focus, readable code and inline collaboration states", () => {
  ...
  assert.match(css, /:focus-visible/); assert.match(css, /font: 13px\/1\.65 var\(--bb-mono\)/);
