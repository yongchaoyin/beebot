...
test("real transcript retains its DOM, closes menus and separates uncertain from retryable delivery", async () => {
  const compiled = await build({
    stdin: { contents: `import React from "react"; import {createRoot} from "react-dom/client"; import {flushSync} from "react-dom";
      ...
      window.fixture={events,render(delivery){flushSync(()=>root.render(<ConversationTranscript entries={[{...original,delivery,deliveryFailure:delivery==="failed"?"rejected":undefined}]} onReply={()=>events.push("reply")} onCopyMessage={()=>events.push("copy")} onResendFailedSend={()=>events.push("resend")} onCancelQueuedSend={()=>events.push("cancel")} onCheckSendReceipt={async()=>{events.push("check");throw Error("Receipt still unknown; nothing resent.")}} />));},unmount(){flushSync(()=>root.unmount());}};`,
