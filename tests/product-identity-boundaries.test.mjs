...
import { patchOriginalSettingsPanel, patchOriginalSettingsRegistry } from "../scripts/lib/router-renderer-patch.mjs";
...
test("legacy feedback errors cannot imply BeeBot subscription or change the error-code contract", async () => {
  const { EXTERNAL_FEEDBACK_MESSAGES } = await import("../source/shared/product-access-copy.ts");
  function table(source) {
    let expression;
    simple(parse(source, { ecmaVersion: "latest", sourceType: "module" }), {
      VariableDeclarator(n) { if (n.id?.name === "OVn") expression = source.slice(n.init.start, n.init.end); },
    });
    assert.ok(expression, "real feedback errors remain present");
    return new Function("xSe", `return (${expression});`)("Grok Bot");
  }
  const beforeTable = table(original), afterTable = table(patchProductAccessCopy(original));
  assert.deepEqual(Object.keys(afterTable).sort(), Object.keys(beforeTable).sort());
  assert.deepEqual(afterTable, EXTERNAL_FEEDBACK_MESSAGES);
  for (const code of ["access-denied", "not-signed-in", "subscription-required"]) {
    assert.match(afterTable[code], /external provider/);
    assert.doesNotMatch(afterTable[code], /Ultra|Premium|Sign in to BeeBot/);
  }
  for (const code of ["invalid-feedback", "rate-limited", "unavailable"]) assert.equal(afterTable[code], beforeTable[code]);
  assert.throws(() => patchProductAccessCopy(original.replace('"subscription-required":"Upgrade to Ultra before sending feedback."', '"subscription-required":"Changed provider contract"')), /differs from the reviewed/);
  const editable = await readFile("frontend/src/production/ProductionRenderer.tsx", "utf8");
  assert.match(editable, /const FEEDBACK_ERRORS: Readonly<Record<FeedbackCode, string>> = EXTERNAL_FEEDBACK_MESSAGES/);
});

test("source, packaged settings and Agent guidance expose Usage without a retired billing promise", async () => {
  const patched = patchOriginalSettingsRegistry(original);
  const sections = new Function(`${patched.match(/const wDn=\[[^;]+?\]/)[0]};return wDn;`)();
  assert.equal(sections.find(section => section.id === "usage").label, "Usage");
  assert.equal(sections.find(section => section.id === "usage").icon, "chart-bars");
  const view = await readFile("frontend/src/recovered/features/settings/overlay/view.tsx", "utf8");
  const commands = await readFile("frontend/src/production/ProductionRenderer.tsx", "utf8");
  const help = await readFile("source/host/runner/box-reference-docs.ts", "utf8");
  assert.match(view, /id: "usage", label: "Usage"/);
  assert.match(commands, /id: "usage", label: "Usage"/);
  assert.doesNotMatch(view + commands + help, /Usage & Billing|Upgrade to Ultra/);
  assert.match(help, /Router, Usage \(when available\)/);
});
