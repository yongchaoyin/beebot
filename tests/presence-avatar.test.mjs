...
test("separately evaluated renderer bundles share one document animation budget and preference", async t => {
  const r=rig(t), first=r.avatar({state:"thinking",identity:"a"});
  const second=await import(new URL("../frontend/src/presence/avatar-motion.ts?consolidation-bundle",import.meta.url));
  const svg=createCharacterSvg(r.document,"blob","blue",32);r.document.body.append(svg);
  const handle=second.registerAvatarMotion(svg,{state:"thinking",identity:"b",size:32});t.after(()=>handle.destroy());
  assert.equal(r.observers.length,1,"native and remote chunks must not each start observers");
  r.visible(first.svg);r.visible(svg);assert.equal(r.timers.size,1);
  second.setAvatarMotionPreference(r.document,"off");
  assert.equal(getAvatarMotionPreference(r.document),"off");assert.equal(r.timers.size,0);
  setAvatarMotionPreference(r.document,"subtle");
  assert.equal(second.getAvatarMotionPreference(r.document),"subtle");assert.equal(r.timers.size,1);
});
